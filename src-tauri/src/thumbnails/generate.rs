use std::path::{Path, PathBuf};
use sha2::{Digest, Sha256};

const THUMB_SIZE: u32 = 400;

fn cache_key(path: &Path, mtime: u64, size: u64) -> String {
    let mut hasher = Sha256::new();
    hasher.update(path.to_string_lossy().as_bytes());
    hasher.update(mtime.to_le_bytes());
    hasher.update(size.to_le_bytes());
    hasher.update(THUMB_SIZE.to_le_bytes());
    hasher.finalize().iter().map(|b| format!("{b:02x}")).collect()
}

pub fn thumb_path(cache_dir: &Path, path: &Path, mtime: u64, size: u64) -> PathBuf {
    let key = cache_key(path, mtime, size);
    cache_dir.join(format!("{key}.jpg"))
}

pub fn generate_thumbnail(
    source: &Path,
    dest: &PathBuf,
    _mtime: u64,
    _size_bytes: u64,
) -> Result<(), String> {
    if dest.exists() {
        return Ok(());
    }

    let ext = source
        .extension()
        .and_then(|e| e.to_str())
        .unwrap_or("")
        .to_lowercase();

    // HEIC/HEIF: copy a placeholder icon instead of decoding
    if ext == "heic" || ext == "heif" {
        return write_heic_placeholder(dest);
    }

    let img = image::open(source).map_err(|e| format!("Failed to open image: {e}"))?;
    let thumb = img.thumbnail(THUMB_SIZE, THUMB_SIZE);

    if let Some(parent) = dest.parent() {
        std::fs::create_dir_all(parent).map_err(|e| format!("Failed to create cache dir: {e}"))?;
    }

    thumb
        .save_with_format(dest, image::ImageFormat::Jpeg)
        .map_err(|e| format!("Failed to save thumbnail: {e}"))
}

fn write_heic_placeholder(dest: &PathBuf) -> Result<(), String> {
    if let Some(parent) = dest.parent() {
        std::fs::create_dir_all(parent).map_err(|e| format!("Failed to create cache dir: {e}"))?;
    }
    // 1×1 grey JPEG as placeholder
    let placeholder: &[u8] = &[
        ,
    ];
    std::fs::write(dest, placeholder).map_err(|e| format!("Failed to write placeholder: {e}"))
}
